...
GCodeResult GCodes::I2cForward(const GCodeBuffer & gb, const uint8_t addr, const uint8_t *data, const size_t dataSize, const StringRef& reply)
{
	...
	if (gbBuffer[1] == ' ' || gbBuffer[1] == '\0') glen = 1;
	else if (gbBuffer[2] == ' ' || gbBuffer[2] == '\0') glen = 2;
	else if (gbBuffer[3] == ' ' || gbBuffer[3] == '\0') glen = 3;
	else if (gbBuffer[4] == ' ' || gbBuffer[4] == '\0') glen = 4;
	else return GCodeResult::error;

	platform.InitI2c();

	// Build a buffer to send over i2c
	const size_t numToWrite = glen + 1 + dataSize;
	uint8_t bufToSend[MaxI2cBytes];
	std::copy_n(gbBuffer, glen, bufToSend);
	bufToSend[glen] = ' ';
	std::copy_n(data, dataSize, bufToSend + glen + 1);
	bufToSend[numToWrite] = '\0';

	size_t bytesSent = 0;
	{
		MutexLocker lock(Tasks::GetI2CMutex());
		bytesSent = I2C_IFACE.Transfer((uint16_t)addr, bufToSend, numToWrite, 0);
	}

	if (bytesSent != numToWrite)
	{
