...
constexpr float DefaultAnchorA[3] = {    0.0, -1437.13, -234.0};
constexpr float DefaultAnchorB[3] = { 1244.59,  718.56, -234.0};
constexpr float DefaultAnchorC[3] = {-1244.59,  718.56, -234.0};

// originally 71. Now 59.5, From anchor to printsurface.
constexpr float fromAnchorToPrintSurface = 59.5;

constexpr float offsetF = DefaultAnchorC[2] - 59.5;
...
inline float HangprinterKinematics::LineLengthSquared(const float machinePos[3], const float anchor[3]) const
{
	...
	// The angle in which the ABC lines connect to the end effector is constant. Hence in the following pythagoras calculation
	// the anchor[Z_AXIS] stays the same at any given moment.
...
bool HangprinterKinematics::CartesianToMotorSteps(const float machinePos[], const float stepsPerMm[], size_t numVisibleAxes, size_t numTotalAxes, int32_t motorPos[], bool isCoordinated) const
{
	...
	char* axixes[5] = {"A axis", "B axis", "C axis", "D axis", "V axis"};
	...
	{
		if(i > 3){
			// The v axis is almost the same as D axis's z value. Only some offset needs to be added for the calculation.
